...
$.widget( "ui.iviewer", $.ui.mouse, {
	...
	showNextPrev: function(){
		//show prev button or not
		if($('a.image-view.active').prev('a.image-view').length <= 0){
			$('.iviewer_rotate_pageup').show();
			$('.iviewer_rotate_pageup').not('.disabled').hide();
		}else{
			$('.iviewer_rotate_pageup').hide();
			$('.iviewer_rotate_pageup').not('.disabled').show();
		}
		//show next button or not
		if($('a.image-view.active').next('a.image-view').length <= 0){
			$('.iviewer_rotate_pagedown').show();
			$('.iviewer_rotate_pagedown').not('.disabled').hide();
		}else{
			$('.iviewer_rotate_pagedown').hide();
			$('.iviewer_rotate_pagedown').not('.disabled').show();
		}
	},
    ...
    **/
    createui: function()
    {
		...
		$("<div>", { "class": "iviewer_common iviewer_button iviewer_rotate_pageup" })
					.bind("mousedown touchstart",function(e) {
						e.preventDefault();
                        $('#div-image-view-resizable-loading').show();
						var pic_src = $('a.image-view.active').removeClass("active")
								.prev('a.image-view').addClass("active").find('img').data('url');
                        var pic_small = pic_src + '&compress=true';
                        //change download url
                        $('#div-image-view-drag').find('a').attr('download',pic_src.substring(pic_src.lastIndexOf('/') + 1));
                        $('#div-image-view-drag').find('a').attr('href',pic_src);
                        $(this).siblings('img').attr("src",pic_small);
		...
		$("<div>", { "class": "iviewer_common iviewer_button iviewer_rotate_pageup disabled" })
					.appendTo(this.container);
					
		$("<div>", { "class": "iviewer_common iviewer_button iviewer_rotate_pagedown" })
					.bind("mousedown touchstart", function(e) {
						e.preventDefault();
                        $('#div-image-view-resizable-loading').show();
						var pic_src = $('a.image-view.active').removeClass("active")
								.next('a.image-view').addClass("active").find('img').data('url');
                        var pic_small = pic_src + '&compress=true';
                        //change download url
                        $('#div-image-view-drag').find('a').attr('download',pic_src.substring(pic_src.lastIndexOf('/') + 1));
                        $('#div-image-view-drag').find('a').attr('href',pic_src);
		...
		$("<div>", { "class": "iviewer_common iviewer_button iviewer_rotate_pagedown disabled" })
					.appendTo(this.container);
					
		this.showNextPrev(); //initial show next/prev button
...
(function() {
    ...
    this.load = function(src, loaded, error) {
        ...
        setTimeout(function () {
            var src_small = src.replace(".","-140x140.")
                .replace(_crm_url_img_view,_url_small_img_view)
